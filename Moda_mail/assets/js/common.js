...
(function (FECommon) {
	...
	var header = {
		...
		,fn_navClose:function(){
			//主選單自動關閉
			if(!windowWidthUnder992 && $navbarSwitch.attr('aria-expanded')=='true'){
	...
	var footer = {
		...
		,fn_ftNavSwitch: function(){
			function openFtNav(e){
				//開選單
				$ftNavSub.stop().slideDown(350).css('display','flex');
				...
				e.attr('title',e.data('closetitle')).text('Close');
			}
			function closeFtNav(e){
				//關選單
				$ftNavSub.stop().slideUp(350);
				e.attr('title',e.data('opentitle')).text('Open');
				setTimeout(function(){
					$ftNav.addClass('off');
					$('.ftNavLt > div').removeClass().addClass('col');
				},250);
			}
			($ftNav.hasClass('off'))?closeFtNav($ftNavSwitch):openFtNav($ftNavSwitch);
			$ftNavSwitch.on('click',function() {
		...
		,fn_ftSubNavStyle:function(){
			if($('.ftSubNavJs > a').length >= 6){
				$('.ftSubNavJs').addClass('itemMoreThen6');
			}else{
				$('.ftSubNavJs').removeClass('itemMoreThen6');
			}
		}
	}
	...
	var widget = {
		...
		,fn_idxList2Swiper:function(){
			var idxList2 = $('.idxList2');
			var idxList2Swiper;
			var sectionTitle;
			// var swiperWrap = $('.idxList2SwiperJs');
			var swiperItem = $('.idxList2SwiperJs .swiper-slide');
			setSwiper();
			...
			function setSwiper(){
				...
				idxList2Swiper = new Swiper('.idxList2SwiperJs', {
					pagination: {
						el: '.idxList2SwiperPageJs'
						,clickable:true
						,renderBullet: function (index, className) {
							return '<span class="' + className + '"><i>第'+ (index+1)+'組'+sectionTitle+'</i></span>';
						}
			...
			function removeSwiper(){
				...
				setTimeout(function(){
					$('.idxList2SwiperJs .swiper-wrapper').css('transform','none');
				},50);
			}
		}
